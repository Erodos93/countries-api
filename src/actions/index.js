...
export const fetchData = () =>async dispatch=>{
  const response = await jsonCountryInfo.get(`/all ? access_key =3c267c46f88544700c8dee91ee211c65`);
  dispatch({
    type:"FETCH_DATA",
...
await jsonCountryInfo.get(`/name/${value} ? access_key =3c267c46f88544700c8dee91ee211c65`).then(res=>{
  console.log(res);
  if (res.status!==404) {
...
export const filterDataByRegion = region =>async dispatch=>{
  const response= await jsonCountryInfo.get(`/region/${region} ? access_key =3c267c46f88544700c8dee91ee211c65`);
