...
const renderCountryPlace=()=>{
          ...
          const loadingConditions=(Object.keys(countries).length===0||Object.keys(countries).length===250) && context.term.length>1;

          return (
            loadingConditions?<div className="loading-item"><div className="loading__title">Loading...</div></div>:<div className="country-list__cards" >
   ...
   return (
     <section className={`country-list  ${state?" light-background dark-text":" dark-background light-text"}`} style={{height:countries&&context.term.length>1?250+"vh":""}} >
     {renderCountryPlace()}
     </section>
   );
}
